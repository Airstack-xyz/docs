...
You can provide the user's fname, e.g. `vitalik.eth`, into the `profileName` input filter and fetch the Farcaster profile image from the response with the `profileImage` field:
...
query MyQuery {
  Socials(
    input: {filter: {profileName: {_eq: "vitalik.eth"}, dappName: {_eq: farcaster}}, blockchain: ethereum}
  ) {
    Social {
      profileImage
    }
